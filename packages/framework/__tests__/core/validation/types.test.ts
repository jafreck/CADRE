...
import type { ValidationResult, PreRunValidator } from '../../../src/core/validation/types.js';
...
describe('PreRunValidator', () => {
  it('should expose a name string', () => {
    const validator: PreRunValidator = {
      name: 'my-validator',
      validate: async (_config: unknown) => ({ passed: true, errors: [], warnings: [] }),
    };
  ...
  it('should return a ValidationResult promise from validate()', async () => {
    const validator: PreRunValidator = {
      name: 'test-validator',
      validate: async (_config: unknown) => ({
        passed: false,
        errors: ['bad config'],
        warnings: [],
      }),
    ...
    const result = await validator.validate({ projectName: 'test' });
    expect(result.passed).toBe(false);
